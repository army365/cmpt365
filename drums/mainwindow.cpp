    ...
    mSerializer(nullptr)
{
    ...
    mSerializer = std::make_unique<FinalSerializer>();
...
void MainWindow::initSoundEffectWidgets()
{
    ...
    ui->kickWidget->loadSound(QUrl("qrc:/res/sounds/kick.wav"));

    ui->snareWidget->setName("Snare");
    ui->snareWidget->setTriggerKey(Qt::Key_J);
    ui->snareWidget->loadSound(QUrl("qrc:/res/sounds/snare.wav"));

    ui->hihatWidget->setName("Hihat");
    ui->hihatWidget->setTriggerKey(Qt::Key_K);
    ui->hihatWidget->loadSound(QUrl("qrc:/res/sounds/hihat.wav"));

    ui->crashWidget->setName("Crash");
    ui->crashWidget->setTriggerKey(Qt::Key_L);
    ui->crashWidget->loadSound(QUrl("qrc:/res/sounds/crash.wav"));
}
...
void MainWindow::startPlayback()
{
    ...
    mplayback = new playback(mTrack);
    mplayback->moveToThread(mPlaybackThread);

    connect(mPlaybackThread, &QThread::started,
            mplayback, &playback::play);
    connect(mPlaybackThread, &QThread::finished,
            mplayback, &QObject::deleteLater);

    connect(mplayback, &playback::playSound,
            this, &MainWindow::playSoundEffect);

    connect(mplayback, &playback::trackFinished,
            &mTrack, &Track::stop);
